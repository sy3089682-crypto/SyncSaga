...
export default function AnimeDetailPage() {
  ...
  if (!media) {
    return (
      <div className="min-h-screen bg-canvas flex items-center justify-center">
        <p className="text-ink-mute">Anime not found</p>
      </div>
    );
  }
    ...
    <div className="min-h-screen bg-canvas text-ink pb-20">
      <div className="relative h-[40vh] sm:h-[50vh] overflow-hidden">
        {media.bannerImage ? (
          ...
          <div className="w-full h-full bg-gradient-to-br from-amber/40 via-amber-hover/20 to-amber/30" />
        )}
        ...
        <div className="absolute bottom-0 left-0 right-0 p-6 sm:p-8">
          <div className="max-w-7xl mx-auto flex items-end gap-6">
            <div className="w-28 sm:w-36 rounded-xl overflow-hidden shadow-2xl border border-border shrink-0 hidden sm:block">
            ...
            <div className="flex-1 min-w-0">
              ...
              {media.title?.native && (
                <p className="text-sm text-ink-soft mb-3">{media.title.native}</p>
              )}
              <div className="flex items-center gap-3 flex-wrap">
                ...
                {media.format && (
                  <span className="px-2 py-0.5 rounded bg-elevated text-xs uppercase">{media.format}</span>
                )}
                {media.episodes && (
                ...
                {media.status && (
                  <span className="flex items-center gap-1 text-xs text-ink-soft capitalize">
          ...
          <div className="flex-1 min-w-0 space-y-8">
            <div className="flex items-center gap-3">
              <Link href={`/room/create?animeId=${media.id}&title=${encodeURIComponent(title)}`}
                className="flex items-center gap-2 px-6 py-3 rounded-xl bg-amber text-canvas font-semibold text-sm hover:bg-amber-hover transition-colors shadow-lg shadow-amber/25">
                <Play className="w-4 h-4" />
                Create Watch Room
              </Link>
              <button className="p-3 rounded-xl bg-surface border border-border text-ink-soft hover:text-ink transition-colors">
                ...
                <div className="flex flex-wrap gap-2">
                  {media.genres.map((g: string) => (
                    <Link key={g} href={`/search?genre=${g}`}
                      className="px-3 py-1.5 rounded-full bg-amber-strong border border-amber/20 text-amber text-sm hover:bg-amber-strong transition-colors">
                      {g}
                    </Link>
                  ))}
                </div>
                ...
                <div className="space-y-1">
                  {episodes.slice(0, episodePage * 25).map((ep: any) => (
                    <Link key={ep.mal_id} href={`/room/create?animeId=${media.id}&episode=${ep.mal_id}&title=${encodeURIComponent(title)}`}
                      className="flex items-center gap-3 p-3 rounded-xl hover:bg-elevated transition-colors group">
                      {ep.thumbnail && (
                      ...
                      <div className="min-w-0 flex-1">
                        ...
                        {ep.air_date && (
                          <p className="text-xs text-ink-mute">{new Date(ep.air_date).toLocaleDateString()}</p>
                        )}
                      </div>
                      <Play className="w-4 h-4 text-ink-mute group-hover:text-amber opacity-0 group-hover:opacity-100 transition-all" />
                    </Link>
                  ))}
                </div>
                {episodes.length > episodePage * 25 && (
                  <button onClick={() => setEpisodePage(p => p + 1)}
                    className="w-full py-3 text-sm text-amber hover:underline mt-2">
                    Show more episodes
                  </button>
                )}
              </section>
                    ...
                    return (
                      <div key={char.character?.mal_id || i} className="flex items-center gap-3 p-3 rounded-xl bg-surface border border-border">
                        ...
                        <div className="min-w-0">
                          ...
                          <p className="text-[10px] text-ink-mute capitalize">{char.role?.toLowerCase()}</p>
                          {va && <p className="text-[10px] text-ink-mute truncate">{va.person?.name}</p>}
                        </div>
                      </div>
                    );
                  })}
          ...
          <aside className="lg:w-80 shrink-0 space-y-4">
            ...
            {media.siteUrl && (
              <a href={media.siteUrl} target="_blank" rel="noopener noreferrer"
                className="flex items-center justify-center gap-2 w-full py-3 rounded-xl bg-surface border border-border text-sm text-ink-soft hover:text-amber hover:border-amber/30 transition-colors">
