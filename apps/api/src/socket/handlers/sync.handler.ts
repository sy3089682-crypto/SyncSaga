...
export function syncHandler(
  ...
  socket.on('sync:event', async (event: SyncEvent) => {
    try {
      ...
      // OPTIMIZATION: Use O(1) user socket id check instead of O(N) array include
  ...
  socket.on('sync:takeover', async ({ roomId }) => {
    try {
      ...
      // OPTIMIZATION: Use O(1) user socket id check instead of fetching all room users
      ...
      // OPTIMIZATION: Use O(1) user socket id check
      const requestingUserSocketId = await redisService.getUserSocketId(roomId, socket.userId);
