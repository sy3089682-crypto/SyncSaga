...
export function syncHandler(
  ...
  socket.on('sync:event', async (event: SyncEvent) => {
    try {
      ...
      // O(1) verify user is in the room
  ...
  socket.on('sync:takeover', async ({ roomId }) => {
    try {
      ...
      // Check if host is actually disconnected (O(1))
      const hostSocketId = await redisService.getUserSocketId(roomId, hostId);
      if (hostSocketId) {
        // Host is still connected to the room — check if their socket is active
      ...
      // Verify requesting user is in the room (O(1))
      const userSocketInRoom = await redisService.getUserSocketId(roomId, socket.userId);
      if (!userSocketInRoom) {
        return socket.emit('error', { code: 'NOT_IN_ROOM', message: 'Not in room' });
      }
