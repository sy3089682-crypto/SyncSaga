...
CREATE TABLE IF NOT EXISTS rooms (
    ...
    sync_mode TEXT DEFAULT 'auto' CHECK (sync_mode IN ('auto', 'manual', 'extension_only')),
    created_at TIMESTAMPTZ DEFAULT NOW(),
    updated_at TIMESTAMPTZ DEFAULT NOW()
);
...
CREATE TABLE IF NOT EXISTS messages (
    ...
    type TEXT DEFAULT 'text' CHECK (type IN ('text', 'gif', 'reaction', 'system', 'scene_comment')),
    scene_id UUID,
    created_at TIMESTAMPTZ DEFAULT NOW()
);
...
CREATE INDEX IF NOT EXISTS idx_scenes_anime ON scenes(anime_id, episode_number);
CREATE INDEX IF NOT EXISTS idx_scenes_fingerprint ON scenes(audio_fingerprint_hash);
...
CREATE INDEX IF NOT EXISTS idx_episodes_anilist ON anime_episodes(anilist_id);
...
CREATE INDEX IF NOT EXISTS idx_landmarks_hash ON fingerprint_landmarks(hash);
CREATE INDEX IF NOT EXISTS idx_landmarks_episode ON fingerprint_landmarks(episode_id);
...
CREATE INDEX IF NOT EXISTS idx_activity_feed_ts ON activity_feed(created_at DESC);
