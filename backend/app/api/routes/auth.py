...
async def get_current_user(authorization: str = Header(None)) -> UserResponse:
    ...
    row = await db.fetchrow('SELECT id, username, display_name, avatar_url FROM profiles WHERE id = $1', uuid.UUID(payload['sub']))
    if not row:
        raise HTTPException(401, 'User not found')
    return UserResponse(id=str(row['id']), username=row['username'], display_name=row['display_name'], avatar_url=row.get('avatar_url'))
...
async def register(req: RegisterRequest):
    ...
    return {'access_token': token, 'token_type': 'bearer', 'user': UserResponse(id=str(user_id), username=req.username, display_name=req.display_name).model_dump()}

@router.post('/login')
async def login(req: LoginRequest):
    row = await db.fetchrow('SELECT id, username, display_name, avatar_url FROM profiles WHERE username = $1', req.username)
    if not row:
    ...
    return {'access_token': token, 'token_type': 'bearer', 'user': UserResponse(id=str(row['id']), username=row['username'], display_name=row['display_name'], avatar_url=row.get('avatar_url')).model_dump()}
