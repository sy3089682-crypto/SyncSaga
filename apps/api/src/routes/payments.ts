...
router.post('/create-checkout', async (req: Request, res: Response) => {
  ...
  try {
    ...
    const { data: existingSub } = await supabase.from('subscriptions').select('stripe_customer_id').eq('user_id', userId).single();
    let customerId = existingSub?.stripe_customer_id;
    if (!customerId) {
      const customer = await stripe.customers.create({ email: profile?.email, metadata: { userId } });
      customerId = customer.id;
      await supabase.from('subscriptions').upsert({ user_id: userId, stripe_customer_id: customerId, plan: 'free', status: 'active' });
    }
    ...
    const session = await stripe.checkout.sessions.create({
      ...
      mode: 'subscription',
      success_url: successUrl || origin + '/settings?payment=success',
      cancel_url: cancelUrl || origin + '/settings?payment=cancelled',
      metadata: { userId },
    });
...
router.post('/portal', async (req: Request, res: Response) => {
  ...
  try {
    ...
    if (!sub?.stripe_customer_id) return res.status(400).json({ error: { code: 'NO_SUBSCRIPTION', message: 'No active subscription' } });
    const origin = req.headers.origin || 'http://localhost:3000';
