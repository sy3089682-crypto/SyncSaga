...
import {
  Star, Play, Calendar, Clock, BookOpen, Users, Tv,
  ChevronDown, ChevronUp, ExternalLink, Loader2, Hash,
} from 'lucide-react';
...
export function AnimeInfoSidebar({
  ...
  const [activeTab, setActiveTab] = useState<'info' | 'episodes' | 'characters'>('info');
  const [characters, setCharacters] = useState<any[]>([]);
  const [themes, setThemes] = useState<{ openings: string[]; endings: string[] } | null>(null);
      ...
      <div className="relative h-40 shrink-0 overflow-hidden">
        ...
        <div className="absolute bottom-3 left-3 right-3 flex items-end gap-3">
          ...
          <div className="min-w-0 flex-1">
            <h3 className="text-sm font-semibold truncate text-white drop-shadow-lg">{title}</h3>
            <div className="flex items-center gap-2 mt-1">
              ...
              {media.format && (
                <span className="text-[10px] px-1.5 py-0.5 rounded bg-white/10 text-text-secondary uppercase">{media.format}</span>
              )}
              {media.episodes && (
                <span className="text-[10px] text-text-muted">{media.episodes} eps</span>
              )}
            </div>
          </div>
        </div>
      ...
      <div className="flex-1 overflow-y-auto">
        <AnimatePresence mode="wait">
            ...
            <motion.div key="info" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="p-3 space-y-3">
              ...
              <div className="grid grid-cols-2 gap-2">
                ...
                {media.season && media.seasonYear && (
                  <div className="flex items-center gap-1.5 text-xs text-text-secondary">
                    ...
                    <span>{media.season.charAt(0) + media.season.slice(1).toLowerCase()} {media.seasonYear}</span>
                  </div>
                )}
                {media.duration && (
                  <div className="flex items-center gap-1.5 text-xs text-text-secondary">
              ...
              {media.studios?.nodes && media.studios.nodes.length > 0 && (
                <div>
                  ...
                  <p className="text-xs">{(media.studios.nodes as any[]).filter((s: any) => s.isAnimationStudio)[0]?.name || (media.studios.nodes as any[])[0]?.name}</p>
                </div>
              )}
              ...
              {nextEpisode && (
                <div className="p-2 rounded-lg bg-accent-cyan/10 border border-accent-cyan/20">
                  <p className="text-xs text-accent-cyan font-medium">
                    Ep {nextEpisode.episode} airs in {Math.floor(nextEpisode.timeUntilAiring / 3600)}h {Math.floor((nextEpisode.timeUntilAiring % 3600) / 60)}m
                  </p>
                </div>
              )}
              ...
              {onSetEpisode && media.episodes && (
                <button onClick={() => onSetEpisode(media.id, 1)}
          ...
          {activeTab === 'episodes' && (
            <motion.div key="episodes" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}>
              <button onClick={() => setShowEpisodes(!showEpisodes)}
                ...
                <span>Episode List ({episodes.length || media.episodes || '?'})</span>
                {showEpisodes ? <ChevronUp className="w-3 h-3" /> : <ChevronDown className="w-3 h-3" />}
              </button>
              <div className="max-h-60 overflow-y-auto">
                {(showEpisodes ? episodes : episodes.slice(0, 10)).map((ep: any, i: number) => (
                  <button key={ep.mal_id || i} onClick={() => onSetEpisode?.(media.id, ep.mal_id ? parseInt(ep.mal_id) : i + 1)}
                    className={cn(
                      'w-full flex items-center gap-3 px-3 py-2 text-left hover:bg-surface-light transition-colors border-b border-border/50',
                      (ep.mal_id ? parseInt(ep.mal_id) : i + 1) === currentEpisode && 'bg-primary/10'
                    )}>
                    <span className="w-6 h-6 rounded-full bg-surface-light flex items-center justify-center text-[10px] text-text-muted shrink-0">
                      {ep.mal_id || i + 1}
                    </span>
                    <div className="min-w-0 flex-1">
                      <p className="text-xs truncate">{ep.title || `Episode ${ep.mal_id || i + 1}`}</p>
                      {ep.air_date && <p className="text-[10px] text-text-muted">{new Date(ep.air_date).toLocaleDateString()}</p>}
                    </div>
                    {ep.thumbnail && (
          ...
          {activeTab === 'characters' && (
            <motion.div key="characters" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="p-2 space-y-2">
              {characters.length === 0 && (
                <p className="text-xs text-text-muted text-center py-4">No character data available</p>
              )}
              {characters.map((char: any, i: number) => {
                const va = char.voice_actors?.[0];
                return (
                  <div key={char.character?.mal_id || i} className="flex items-center gap-3 p-2 rounded-lg hover:bg-surface-light transition-colors">
                    <img src={char.character?.images?.jpg?.image_url} alt={char.character?.name}
                      className="w-8 h-8 rounded-full object-cover shrink-0" />
                    <div className="min-w-0 flex-1">
                      <p className="text-xs font-medium truncate">{char.character?.name}</p>
                    ...
                    {va && (
                      <div className="flex items-center gap-1.5 shrink-0">
                        ...
                        <span className="text-[10px] text-text-muted hidden md:inline truncate max-w-[60px]">{va.person?.name}</span>
                      </div>
                    )}
                  </div>
                );
      ...
      {themes && (themes.openings.length > 0 || themes.endings.length > 0) && (
        <div className="border-t border-border p-3 shrink-0">
          {themes.openings.length > 0 && (
            <div className="mb-1">
              <span className="text-[10px] uppercase text-text-muted tracking-wider">OP</span>
              <p className="text-xs truncate" title={themes.openings[0]}>{themes.openings[0]}</p>
            </div>
          )}
          {themes.endings.length > 0 && (
            <div>
              <span className="text-[10px] uppercase text-text-muted tracking-wider">ED</span>
              <p className="text-xs truncate" title={themes.endings[0]}>{themes.endings[0]}</p>
            </div>
          )}
        </div>
      )}
