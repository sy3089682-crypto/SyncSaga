            ...
            {tabs.map(tab => (
              <button key={tab.id} onClick={() => setActiveTab(tab.id)}
                className={cn(
                  ...
                  activeTab === tab.id ? 'bg-amber-strong text-amber' : 'text-ink-soft hover:text-ink hover:bg-elevated'
                )}>
              ...
              <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} className="space-y-6">
                <div className="glass rounded-2xl p-6 border border-border/50">
                  ...
                  <div className="flex items-center gap-4 mb-6">
                    <div className="w-16 h-16 rounded-full bg-gradient-to-br from-amber to-amber-hover flex items-center justify-center text-2xl font-bold">
                  ...
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                    <div>
                      ...
                      <input type="text" defaultValue={user?.username || ''} readOnly
                        className="w-full px-4 py-2.5 rounded-xl bg-elevated border border-border text-ink" />
                    </div>
                    <div>
                      <label className="block text-sm text-ink-soft mb-1">Display Name</label>
                    ...
                    {(['dark', 'light', 'amoled', 'sakura'] as Theme[]).map(t => (
                      <button key={t} onClick={() => setTheme(t)}
                        className={cn(
                          ...
                          theme === t ? 'border-amber bg-amber/10' : 'border-border bg-elevated hover:border-amber/50'
                        )}>
                        <div className={cn(
                  ...
                  {loading ? (
                    ...
                    <div className="space-y-6">
                      <div className={cn(
                        ...
                        subscription?.plan === 'free' ? 'border-border bg-elevated' :
                      ...
                      <div className="grid grid-cols-1 sm:grid-cols-3 gap-3">
                        {[
                          ...
                          <div key={plan.name} className={cn(
                            ...
                            subscription?.plan === plan.name.toLowerCase() ? 'border-amber bg-amber/10' : 'border-border bg-elevated'
                          )}>
                            <p className="font-bold text-lg">{plan.name}</p>
                            <p className="text-2xl font-bold my-2">{plan.price}<span className="text-sm text-ink-soft">/mo</span></p>
                            ...
                            {subscription?.plan !== plan.name.toLowerCase() && plan.name !== 'Free' && (
                              <button onClick={handleUpgrade} disabled={updating}
                                className="w-full py-2 rounded-lg bg-amber text-canvas text-sm font-medium hover:bg-amber-hover transition-colors disabled:opacity-50">
                  ...
                  <div className="space-y-4">
                    {[
                      ...
                      <label key={item.label} className="flex items-center justify-between py-2">
                        ...
                        <div className="w-10 h-6 rounded-full bg-elevated border border-border relative cursor-pointer">
                          <div className="w-4 h-4 rounded-full bg-amber absolute top-0.5 right-0.5 transition-all" />
                        </div>
                      </label>
                    ))}
                  </div>
