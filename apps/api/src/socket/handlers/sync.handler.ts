...
export function syncHandler(
  ...
  socket.on('sync:event', async (event: SyncEvent) => {
    try {
      ...
      // ⚡ Bolt Optimization: Use O(1) getUserSocketId instead of O(N) getRoomUsers for faster presence check
      const inRoom = await redisService.getUserSocketId(roomId, socket.userId);
      if (!inRoom) {
  ...
  socket.on('sync:takeover', async ({ roomId }) => {
    try {
      ...
      // ⚡ Bolt Optimization: Directly query host socket rather than loading all room users
      ...
      // ⚡ Bolt Optimization: O(1) user presence check
      const inRoom = await redisService.getUserSocketId(roomId, socket.userId);
      if (!inRoom) {
        return socket.emit('error', { code: 'NOT_IN_ROOM', message: 'Not in room' });
      }
