...
export function syncHandler(
  ...
  socket.on('sync:event', async (event: SyncEvent) => {
    try {
      ...
      // O(1) presence check instead of O(N) getRoomUsers().includes()
  ...
  socket.on('sync:takeover', async ({ roomId }) => {
    try {
      ...
      // O(1) check if host is still connected
      ...
      // O(1) verify requesting user is in the room
      const requesterSocketId = await redisService.getUserSocketId(roomId, socket.userId);
