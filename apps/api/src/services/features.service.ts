...
const FLAGS: FlagConfig[] = [
  ...
  {
    key: "ai_recaps",
    description: "AI watch party recaps",
    defaultEnabled: true,
  },
  {
    key: "ai_room_names",
    description: "AI room name generation",
    defaultEnabled: true,
  },
  { key: "ai_search", description: "AI-powered search", defaultEnabled: true },
  {
    key: "ai_summaries",
    description: "AI session summaries",
    defaultEnabled: true,
  },
  {
    key: "ai_chat_assistant",
    description: "AI chat assistant (SyncBot)",
    defaultEnabled: true,
  },
  {
    key: "ai_embeddings",
    description: "AI vector embeddings",
    defaultEnabled: true,
    requiresEnv: "CLOUDFLARE_ACCOUNT_ID",
  },
  {
    key: "extension_diagnostics",
    description: "Extension diagnostic tools",
    defaultEnabled: true,
  },
  {
    key: "voice_chat",
    description: "Voice chat via LiveKit",
    defaultEnabled: true,
    requiresEnv: "LIVEKIT_API_KEY",
  },
  {
    key: "clips",
    description: "Clip creation and sharing",
    defaultEnabled: true,
  },
  ...
  },
  {
    ...
    defaultEnabled: true,
    requiresEnv: "POSTHOG_API_KEY",
  },
  {
    key: "embed",
    description: "Embedded rooms for external sites",
    defaultEnabled: true,
  },
  {
    key: "soundboard",
    description: "Soundboard effects",
    defaultEnabled: true,
  },
  {
    key: "taste_graph",
    description: "Taste matching graph",
    defaultEnabled: true,
  },
];
...
class FeatureService {
  ...
  async setOverride(flag: FeatureFlag, enabled: boolean) {
    try {
      await redisService
        .getClient()
        .set(`${OVERRIDE_PREFIX}${flag}`, enabled ? "true" : "false", {
          EX: 86400 * 7,
        });
    } catch {}
  }
  ...
  async isEnabled(flag: FeatureFlag): Promise<boolean> {
    try {
      const overrideVal = await redisService
        .getClient()
        .get(`${OVERRIDE_PREFIX}${flag}`);
  ...
  async getEnabledFeatures(): Promise<FeatureFlag[]> {
    const results = await Promise.all(
      FLAGS.map(async (f) => ({
        key: f.key,
        enabled: await this.isEnabled(f.key),
      })),
    );
    return results.filter((f) => f.enabled).map((f) => f.key);
  }

  async isAvailableForPlan(
    flag: FeatureFlag,
